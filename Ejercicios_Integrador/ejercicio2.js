...
function verificar(orden, callback) {
  setTimeout(() => {
    console.log(`1. Verificada orden ${orden.id}`);
    callback(orden);
  }, 1500);
}
...
function procesar(orden, callback) {
  setTimeout(() => {
    console.log(`2. Procesada orden ${orden.id}`);
    callback(orden);
  }, 2000);
}
...
function registrar(orden, callback) {
  setTimeout(() => {
    console.log(`3. Registrada orden ${orden.id}`);
    callback(orden);
  }, 1000);
}
...
function notificar(orden, callback) {
  setTimeout(() => {
    console.log(`4. Notificada orden ${orden.id}`);
    callback(orden);
  }, 500);
}

// Ejecución anidada (Callback Hell):
console.time("Tiempo Callback");
verificar(orden, (o1) => {
  procesar(o1, (o2) => {
    registrar(o2, (o3) => {
      notificar(o3, (o4) => {
